...
//$sql = $conn->query("SELECT EXISTS(SELECT * FROM areas WHERE areCodigo = $id)");
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <div id="contenido2">
      <section>
        <article id="mcontenido2">
          ...
          <form action="index.php?accion=modificarArea" method="post" id="mcontenido2form" onkeydown="return event.key != 'Enter';" style="width: 47%;height: 210px;padding: 35px 40px;">
            <table>
              <tr>
                <td>Cod. Área: </td>
                <td><input type="text" name="modiarea" id="modiarea"></td>
              </tr>
              <tr>
                <td colspan="2">
                  <center><input type="submit" name="ingresarAreaEnviar" value="Modificar" id="ingresarAreaEnviar"></center></td>
                </td>
              </tr>
            </table>
          </form>
          <img src="Vista/img/insert.png" alt="libreria" style="border-radius: -10;width: 46%;height: 85%;margin: -214px -78px;float: right;border-radius: 15px;">        </article>
        </article>
      </section>
    </div>
